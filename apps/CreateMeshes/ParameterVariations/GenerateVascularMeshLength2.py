...
def CreateIdealSkeleton(Directory, Generations,GenerationsX,Height,HorizonatalEdgeLength, theta, Translation, CollapseBranch, Bounds):
    ...
    if CollapseBranch:
        for i in Nodes:
            if len(i)>1:
                counter =  counter +1
                if (i[1] ==0 and  i[0]>LeftBound-0.01 and i[0]< RightBound+0.01):
...
if __name__=="__main__":
    ...
    Directory = "/data/vascrem/MeshCollection/IdealisedNetwork/3X2Network/AlteringLength/"
    ...
    Length = [1.2,1.4,1.6,1.8,2]#,3.2,3.4,3.6,3.8,4,4.2,4.4,4.6,4.8,5]
    for j in Length:
        ...
        print Bounds
        Bound3  = 5* HorizonatalEdgeLength+4*xia-0.1
        ends = [0.4,Bound3]
        ...
        for i in Collapse:
            ...
            command = CPP_Centerlines_vtp_writer + ' -ofile ' + CenterLines_filename + ' -CenterlinePoints ' +LengthDirectory+ 'CenterlinePoints.txt -CenterlineEdges ' + Directory +'CenterlineEdges.txt -Radius 0.2 -LeftBound '+str(Boundaries[0])+' -RightBound '+str(Boundaries[1])+' -Collapse ' + str(i) + ' -Branch ' + str(0)
