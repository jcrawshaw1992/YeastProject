...
def MakeVertex():
    ...
    print("GG")
    pause()
    writer.SetFileName("Unstructured.vtp")
    print("HH")
    pause()
...
def MakePolyVertex():
    print("MakePolyVertex")
    print("A polyvertex is a cell represents a set of 0D vertices")
    pause()
    ...
    points.InsertNextPoint(1.5, 0.1, 0)
    points.InsertNextPoint(2, 0, 1)
    points.InsertNextPoint(2.5, 0, .4)
    points.InsertNextPoint(3, 0.1, .6)
    pause()
    ...
    polydata_pts.GetPointData().SetScalars(Radii)
    polydata_pts.SetPoints(points)
    # polydata.SetVerts(Vertices)
    writer = vtk.vtkXMLPolyDataWriter()
