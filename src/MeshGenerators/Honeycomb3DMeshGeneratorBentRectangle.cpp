...
Honeycomb3DMeshGeneratorBentRectangle::Honeycomb3DMeshGeneratorBentRectangle(unsigned numNodesLongWidth, unsigned numNodesAlongLength, double width, double length)
  : mpMesh(NULL),
    mMeshFilename("mesh")
{
	TRACE("DING");
    ...
    double z0 = -vertical_spacing * 0;
    double z = 0;
    double MaxX = width/2;
    double MaxY = length/2;
    PRINT_2_VARIABLES(MaxX, horizontal_spacing);
	...
	for (unsigned i=0; i<num_nodes_along_length; i++)
	{
		...
		for (unsigned j=0; j<num_nodes_along_width; j++)
		{
            ...
            // if (x >= MaxX )
            // {
            //     z =  z0 +horizontal_spacing * ((double)j + 0.25 * (1.0 + SmallPow(-1.0, i + 1))) - MaxX ;
            //     x = MaxX ;
            // }
            if (y >= MaxY)
            {
                z =  z0 +vertical_spacing * (double)i- MaxY ;
                y = MaxY ;
            }
...
Honeycomb3DMeshGeneratorBentRectangle::~Honeycomb3DMeshGeneratorBentRectangle()
{
    delete mpMesh;
}

MutableMesh<2,3>* Honeycomb3DMeshGeneratorBentRectangle::GetMesh()
{
