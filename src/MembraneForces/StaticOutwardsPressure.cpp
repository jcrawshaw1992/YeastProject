...
StaticOutwardsPressure::StaticOutwardsPressure()
        : AbstractForce<2, 3>()
{
}

void StaticOutwardsPressure::SetPressure(double Pressure)
{
      mStrength =  Pressure;
}

void StaticOutwardsPressure::AddForceContribution(AbstractCellPopulation<2, 3>& rCellPopulation)
{
                ...
                ++cell_iter)
        {
            ...
            c_vector<double, 3> force = mStrength * mInitialNormals[node_index]; 
            
            rCellPopulation.GetNode(node_index)->AddAppliedForceContribution(force);
            cell_iter->GetCellData()->SetItem("OutwardForce", mStrength);
...
void StaticOutwardsPressure::SetUpNormals(AbstractCellPopulation<2, 3>& rCellPopulation)
{
        HistoryDepMeshBasedCellPopulation<2, 3>* p_cell_population = static_cast<HistoryDepMeshBasedCellPopulation<2, 3>*>(&rCellPopulation);        
        std::map<unsigned, c_vector<double, 3> > ForceMap;
        for (AbstractCellPopulation<2, 3>::Iterator cell_iter = rCellPopulation.Begin();
                ...
                ++cell_iter)
        {
            ...
            mInitialNormals[node_index] = Normal;
...
void StaticOutwardsPressure::OutputForceParameters(out_stream& rParamsFile)
{
...
CHASTE_CLASS_EXPORT(StaticOutwardsPressure)
