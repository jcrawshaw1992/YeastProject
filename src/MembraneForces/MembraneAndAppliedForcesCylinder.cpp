...
void MembraneForcesBasicCylinder::SetPressure(double Pressure)
{
    mPressure = Pressure;
}
void MembraneForcesBasicCylinder::SetCylinderDims(double Nc, double Nz)
{
    mNc = Nc;
    mNz = Nz;
}
...
void MembraneForcesBasicCylinder::AddForceContribution(AbstractCellPopulation<2, 3>& rCellPopulation)
{
    ...
    mMembraneSurface
        ...
        ForceOnNode[0] -= 0.5 * mMembraneSurface * AreaDiff * VectorProduct(UnitNormal, vector_12t);
        
        // Force on Node 1
        c_vector<double, 3> vector_20 = pNode0->rGetLocation() - pNode2->rGetLocation();
        ForceOnNode[1] -= 0.5 * mMembraneSurface * AreaDiff * VectorProduct(UnitNormal, vector_20);

        // Force on Node 2
        c_vector<double, 3> vector_01 = pNode1->rGetLocation() - pNode0->rGetLocation();
        ForceOnNode[2] -= 0.5 * mMembraneSurface * AreaDiff * VectorProduct(UnitNormal, vector_01);
