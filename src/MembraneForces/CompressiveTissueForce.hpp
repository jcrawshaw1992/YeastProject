...
// #ifndef CompressiveTissueForce_HPP_
// #define CompressiveTissueForce_HPP_
...
class CompressiveTissueForce : public AbstractForce<2, 3>
{
private:
    double mStrength;
...
public:
    CompressiveTissueForce(double strength = 1.0)
            : AbstractForce<2, 3>(),
              mStrength(strength)
    {
    ...
    void AddForceContribution(AbstractCellPopulation<2, 3>& rCellPopulation)
    {
             ...
             ++cell_iter)
        {
            ...
            Normal /= norm_2(Normal );
             c_vector<long double, 3> CompressiveForce = -mStrength * Normal;  // / norm_2(cell_location);
         //   PRINT_VECTOR(force);

            rCellPopulation.GetNode(node_index)->AddAppliedForceContribution(CompressiveForce);

            cell_iter->GetCellData()->SetItem("Compressive_force_x", CompressiveForce[0]);
            cell_iter->GetCellData()->SetItem("Compressive_force_y", CompressiveForce[1]);
            cell_iter->GetCellData()->SetItem("Compressive_force_z", CompressiveForce[2]);
            cell_iter->GetCellData()->SetItem("CompressivePressure", norm_2(CompressiveForce));
        }
    }
...
CHASTE_CLASS_EXPORT(CompressiveTissueForce)
