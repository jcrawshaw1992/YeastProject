...
Honeycomb3DMeshGeneratorRand::Honeycomb3DMeshGeneratorRand(unsigned numNodesLongWidth, unsigned numNodesAlongLength, double width, double length)
  : mpMesh(NULL),
    mMeshFilename("mesh")
{
	TRACE("DING");
	...
	for (unsigned i=0; i<num_nodes_along_length; i++)
	{
		...
		for (unsigned j=0; j<num_nodes_along_width; j++)
		{
			...
			unsigned boundary = 0;
			if ((i==0) || (i==numNodesAlongLength-1) || (j==0) ||(j==numNodesLongWidth-1) )
			{
			  boundary = 2;
			}	
            ...
            double x = x0 + horizontal_spacing * ((double)j + 0.25 * (1.0 + SmallPow(-1.0, i + 1))) +rand()/horizontal_spacing ;
            double y = y0 + vertical_spacing * (double)i+rand()/vertical_spacing;
            z = 2*rand()/( vertical_spacing + horizontal_spacing);
...
Honeycomb3DMeshGeneratorRand::~Honeycomb3DMeshGeneratorRand()
{
    delete mpMesh;
}


std::vector<unsigned> Honeycomb3DMeshGeneratorRand::GetBoundaryVector()
{
    return mBoundaryVector;
}

MutableMesh<2,3>* Honeycomb3DMeshGeneratorRand::GetMesh()
{
