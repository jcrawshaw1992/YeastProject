...
void BoundariesModifier<ELEMENT_DIM, SPACE_DIM>::SetupSolve(AbstractCellPopulation<ELEMENT_DIM, SPACE_DIM>& rCellPopulation, std::string outputDirectory)
{
         ...
         ++cell_iter)
    {
        ...
        if (cell_iter->GetCellData()->GetItem("Boundary") == 1)
        {
            ...
            double Distance3 = 0.1;
            double Distance2 = 0.1;
            ...
            // {
            //     TRACE("D")
            //    CellPtr p_cell = rCellPopulation.GetCellUsingLocationIndex(NearestNodes[i]);
            //    p_cell->GetCellData()->SetItem("Boundary",-1);
            //     TRACE("DD")
            // }
...
template <unsigned ELEMENT_DIM, unsigned SPACE_DIM>
c_vector<unsigned, 3> BoundariesModifier<ELEMENT_DIM, SPACE_DIM>::GetNeighbouringNodes(unsigned node_index)
{
    // assert(NodeIsInSet)
...
template <unsigned ELEMENT_DIM, unsigned SPACE_DIM>
std::map<unsigned, c_vector<unsigned, 2> > BoundariesModifier<ELEMENT_DIM, SPACE_DIM>::GetNeighbouringNodesMap()
{
