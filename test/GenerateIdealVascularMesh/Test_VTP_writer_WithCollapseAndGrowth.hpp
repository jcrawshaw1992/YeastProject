...
public:
    ...
    {
         std::string CenterlineEdgeFile; std::string CenterlinePointsFile; std::string output_file;double RadiValue;double Collapse; double Expand;
        ...
        if (CommandLineArguments::Instance()->OptionExists("-CenterlineEdges"))
        {
            ...
            Expand = CommandLineArguments::Instance()->GetDoubleCorrespondingToOption("-Expand");
        ...
        std::vector<double> ExpandedRegion;
        double counter = 0;
        if (PointsFile.is_open())
        {
            while ( getline (PointsFile,line) )
            {
                ...
                {             
                    ...
                    {
                        ExpandedRegion.push_back(counter);
                    }else if(Y <-1  &&  X >3.5 && X<4.3)
                    {
        ...
        for (double vertex_id = 0; vertex_id < points->GetNumberOfPoints(); vertex_id++)
        {
            ...
            }else if (std::find(ExpandedRegion.begin(), ExpandedRegion.end(), vertex_id) != ExpandedRegion.end()) /* v does contain x */
            {
              radii_array->InsertNextValue(RadiValue*Expand);
            }
            else
            {
