    ...
    for(double k = 0; k > -1; k--)
    {
        ...
        for (double j = -1; j > -2; j--)
        {
            ...
            for (int i = 0; i < 5000; i++)
            {
                    ...
                    potts_simulator.SetOutputDirectory("PottsMetrics/Comparision/LongRun/PERIODIC/" + Parameters + "/Trial" + Iteration);
            ...
            for (int i = 0; i < 5000; i++)
            {
                ...
                for (unsigned i = 0; i < num_cells_seeded; ++i) // i+=2)
                {
                    ...
                    double Y = 0;
                    double X = 0;
                    while (OnEdge || X >30 || X < 5 || Y > 30 || Y < 5)
                    {
                ...
                potts_simulator.SetOutputDirectory("PottsMetrics/Comparision/LongRun/NonPERIODIC/" + Parameters + "/Trial" + Iteration);
                potts_simulator.Solve();
                SimulationTime::Instance()->Destroy();
                SimulationTime::Instance()->SetStartTime(0.0);
            }
