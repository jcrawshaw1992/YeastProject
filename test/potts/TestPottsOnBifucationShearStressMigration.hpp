    ...
    void TestPottsOnCylinderWithDeformation()
    {
        ...
        for (unsigned i = 0; i < num_cells_seeded; i++)
        {
            ...
            unsigned num_node = 0;
               ...
               double Y = 2020 ;
                bool Next = 0;
                while (Next ==0 ) // want to grab something on the right side, need to find what seed number i want 
				{
				  num_node = RandomNumberGenerator::Instance()->randMod(p_potts_mesh->GetNumNodes());  // Selects a random node 
                    if ( num_node != X && num_node != Y)
                    {
                        Next =1;
                    }
        ...
        potts_simulator.SetOutputDirectory("PottsBifucationToConflunce2");
        ...
        p_volume_constraint_update_rule->SetDeformationEnergyParameter(4e4);
        ...
        p_area_constraint_update_rule->SetSurfaceAreaEnergyParameter(16e5);
        ...
        TRACE("First solve down, now do second");
       ...
       CellPerimeter/=3;//
        ...
        p_mechanotaxis_update_rule->SetTractionCorrelationParameter(0.016e4); //1e-8);
        potts_simulator.AddUpdateRule(p_mechanotaxis_update_rule);
       

        p_volume_constraint_update_rule->SetDeformationEnergyParameter(4e1);
        p_volume_constraint_update_rule->SetMatureCellTargetVolume(CellArea);
        p_area_constraint_update_rule->SetSurfaceAreaEnergyParameter(16e6);
        p_area_constraint_update_rule->SetTargetSurfaceArea(CellPerimeter);

        potts_simulator.SetEndTime(Confluence_time+20);
