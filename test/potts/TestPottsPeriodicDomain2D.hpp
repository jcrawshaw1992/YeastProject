...
public:
    void TestPottsOnCylinderWithDeformation()
	{
		...
		unsigned max_num_cells =1;  
		unsigned num_cells_seeded = 1;
		double CellScale =1e6;//1e3;


		double LongAxis = 10/2 *10;//* 1e-6 * CellScale ;//*scale ;
		...
		// std::vector<std::pair <unsigned,unsigned> > ElementPairing;
		...
		// for (unsigned i=0; i<num_cells_seeded; i++) 
		{
			...
			unsigned num_node = 500;//RandomNumberGenerator::Instance()->randMod(p_potts_mesh->GetNumNodes()-1);  // Selects a random node 
		...
		// potts_simulator.LabelEdges(N_D, N_Z);
		...
		p_volume_constraint_update_rule->SetMatureCellTargetVolume(CellArea*8);
		...
		// p_potts_mesh->CorrectedPeriodicEdges();
        ...
        potts_simulator.SetOutputDirectory("TestingPeriodicDomian/UnwrappedCylinder");
