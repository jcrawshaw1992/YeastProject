  ...
  void TestNonPeriodicPottsSweep()
    {
		...
		*/
        ...
        for (double k = 10; k > -31; k--)
        {
            for (double j = 10; j > -31; j--)
            {
                ...
                for (int i = 0; i < 100; i++)
                {
                    ...
                    p_volume_constraint_update_rule->SetDeformationEnergyParameter(pow(10, k/10)); //0.1);
                    ...
                    out2 << "Area_" << k/10 << "Perimeter_" << j/10;
                    std::string Parameters = out2.str();

                    potts_simulator.SetOutputDirectory("PottsMetrics/AreaAndPerimineterRefined/NonPeriodic/" + Parameters + "/Trial" + Iteration);
            ...
            for (double j = 10; j > -31; j--)
            {
                ...
                for (int i = 0; i < 100; i++)
                {
                    ...
                    for (unsigned i = 0; i < num_cells_seeded * 2; i += 2)
                    {
                        ...
                        while (OnEdge)
                        {
                            num_node = RandomNumberGenerator::Instance()->ranf() * (p_potts_mesh->GetNumNodes() - 3); // Selects a random node
                            if (p_potts_mesh->GetNode(num_node)->IsBoundaryNode() == 0)
                            {
                                OnEdge = 0;
                            }
                    ...
                    potts_simulator.SetOutputDirectory("PottsMetrics/AreaAndPerimineterRefined/Periodic/" + Parameters + "/Trial" + Iteration);
                    potts_simulator.Solve();
                    SimulationTime::Instance()->Destroy();
                    SimulationTime::Instance()->SetStartTime(0.0);
                }
