...
public:
    void TestHetroPlexus() throw(Exception)
    {
        ...
        std::string Target_mesh_file = "/Users/jcrawshaw/Documents/testoutput/ShrinkingPlexus/LargerBendingForce/results_from_time_0/mesh_10000.vtu";
        ...
        VtkMeshReader<2, 3> mesh_reader2(Target_mesh_file);
        MutableMesh<2, 3> p_mesh_Target;
        p_mesh_Target.ConstructFromMeshReader(mesh_reader2);
        // p_mesh_Target.Scale(1.0 * scale, 1.0 * scale, 1.0 * scale); // so distances are back in original scale
        
        std::string output_directory = "ShrinkingPlexus/CollapsingToTarget/";

        // Create cells
        MAKE_PTR(DifferentiatedCellProliferativeType, p_differentiated_type);
        MAKE_PTR(DifferentiatedCellProliferativeType, p_differentiated_type_T);

        std::vector<CellPtr> cells;
        CellsGenerator<FixedG1GenerationalCellCycleModel, 2> cells_generator;
        cells_generator.GenerateBasicRandom(cells, p_mesh.GetNumNodes(), p_differentiated_type);

        std::vector<CellPtr> cells_T;
        CellsGenerator<FixedG1GenerationalCellCycleModel, 2> cells_generator_T;
        cells_generator_T.GenerateBasicRandom(cells_T, p_mesh_Target.GetNumNodes(), p_differentiated_type_T);

        // Create a cell population
        MeshBasedCellPopulation<2, 3> cell_population(p_mesh, cells);
        MeshBasedCellPopulation<2, 3> cell_population_Target(p_mesh_Target, cells_T);
        ...
        */
        ...
        p_shear_force->SetupMembraneConfiguration(cell_population, cell_population_Target);
        ...
        */
        ...
        // p_membrane_force->SetupInitialMembrane(p_mesh_Target, simulator.rGetCellPopulation());
