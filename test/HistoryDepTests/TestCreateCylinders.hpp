...
void TestCreateBendEllipticalCylinders_WithDiffereingRefinments() throw(Exception)
      ...
      unsigned Refinment[4] = {20,40,100,150};//,160};
        ...
        simulator.Solve();
            ...
            for (int i=0; i<p_mesh->GetNumNodes(); i++)
            {
                ...
                double X = R  * cos(Angle)+ 0.5*InitalLocation[2]*InitalLocation[2]*InitalLocation[2];//+ InitalLocation[2]-1;
 ...
 void Test_CreateAndDeformCollectionOfCylinders_WithDiffereingRefinments() throw(Exception)
    {
