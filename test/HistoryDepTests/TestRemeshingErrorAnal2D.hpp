...
{
public:
    void TestRemeshingSquare() throw(Exception)
    {
        std::vector<double> RootMeanSquared;
        std::vector<double> RootSTDSquared;
        std::vector<double> EdgeL;
        double EdgeLength =2;
        std::string output_dir = "~/Documents/testoutput/RemeshingErrorAnal3D/2DSquare/";
        ...
        std::string offfile = output_dir + "Square.off";
        ...
        // std::system(vtu2offCommand.c_str()); // system only takes char *.
          ...
          double Iter =3;
          while  (EdgeLength >0.22)
          {
            output_dir = "~/Documents/testoutput/RemeshingErrorAnal3D/2DSquare/";
            ...
            std::string CGALRemeshingCommand = "(cd  ~/Documents/CGAL-5.0.2/examples/Polygon_mesh_processing/;./isotropic_remeshing_ForChaste -input " + offfile + " -output " + output_dir + "RemeshedSquare.off -target_edge_length " + std::to_string(EdgeLength) + " -iterations " + std::to_string(Iter) + " )";
            std::system(CGALRemeshingCommand.c_str()); // system only takes char *

            // Now ned to convert from .off back to a .vtu
            std::string Remeshedvtu = output_dir + "RemeshedSquare.vtu";
            std::string off2vtuCommand = "meshio-convert " + output_dir + "RemeshedSquare.off " + Remeshedvtu;
            std::system(off2vtuCommand.c_str()); // system only takes char *

            // Read in the new cylinder that is generated to my desire
            std::string mesh_file ="/Users/jcrawshaw/Documents/testoutput/RemeshingErrorAnal3D/2DSquare/RemeshedSquare.vtu";
            ...
            out << round(EdgeLength*100)/100;
            std::string mesh_size = out.str();
            output_dir = "RemeshingErrorAnal3D/2DSquare/Iterations/" + mesh_size;
            ...
            cell_population.SetTargetRemeshingEdgeLength(0.5);
            ...
            for (int i = 0; i < mesh.GetNumNodes(); i++)
            {
                ...
                // double X =  0.8*(InitalLocation[0]*InitalLocation[0])-1.5;
                // X +=(InitalLocation[1])*(3-InitalLocation[1])/2.5*(1.5-InitalLocation[1])/0.5;
                    // DeformedNodes(:,1)= DeformedNodes(:,1)+ (DeformedNodes(:,2)).*(3-DeformedNodes(:,2))/2.5.*(1.5-DeformedNodes(:,2))/0.5;
                double Y =  InitalLocation[1] * InitalLocation[1] /2;
                double X =  InitalLocation[0] * InitalLocation[0] /2;
                c_vector<double, 3> DeformedLocation = Create_c_vector(X, Y, 0);
            ...
            std::vector<double> Error;
             ...
             for (int i = 0; i < mesh.GetNumNodes(); i++)
            {
                // PRINT_VARIABLE(cell_population.GetNode(i)->GetIndex())
                c_vector<double, 3> IC = InitalNodePos[cell_population.GetNode(i)->GetIndex()];
       
                // double X =  0.8*(IC[0]*IC[0])-1.5;
                // X +=(IC[1])*(3-IC[1])/2.5*(1.5-IC[1])/0.5;
                double Y =  IC[1] * IC[1] /2;
                double X =  IC[0] * IC[0] /2;
                c_vector<double, 3> DeformedLocation_IC = Create_c_vector(X, Y, 0);
            ...
            output_dir = "~/Documents/testoutput/RemeshingErrorAnal3D/2DSquare/Iterations/" + mesh_size;
    ...
    std::vector<double>:: iterator k = EdgeL.begin();
        ...
        std::ofstream f("DeformedSquareErrorAnalsis.txt");
        for(std::vector<double>:: iterator i = RootMeanSquared.begin(); i != RootMeanSquared.end();++j ,++k,  ++i) 
        {
            f <<*k << " "<< *i << " "<< *j <<  '\n';
        }
